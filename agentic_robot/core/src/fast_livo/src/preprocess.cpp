  ...
  if (feature_enabled) {
    for (uint i = 1; i < plsize; i++) {
      if ((msg->points[i].line < N_SCANS) &&
        ...
        double range = pl_full[i].x * pl_full[i].x +
                       pl_full[i].y * pl_full[i].y +
                       pl_full[i].z * pl_full[i].z;
        if (range < blind_sqr || range > max_range) continue;

        bool is_new = false;
        if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) ||
            (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7) ||
            (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
          pl_buff[msg->points[i].line].emplace_back(pl_full[i]);
        }
      }
    }
    static int count = 0;
    ...
    for (uint i = 0; i < plsize; i++) {
      ...
      double range =
          sqrt(pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y +
               pl_full[i].z * pl_full[i].z);
      if (valid_num % point_filter_num == 0) {
        if (range >= blind && range < max_range) {
          pcl_out->emplace_back(pl_full[i]);
        }
      }
      // }
    }
  }
  double cost_time = omp_get_wtime() - start_time;
  printf("[ Preprocess ] Output point number: %zu \n", pcl_out->size());
  // printf("[ Preprocess ] Time cost: %lf \n", cost_time);
}
  ...
  if (feature_enabled) {
    ...
    for (uint i = 0; i < plsize; i++) {
      ...
      if (pl_orig.points[i].ring < N_SCANS) {
        pl_buff[pl_orig.points[i].ring].emplace_back(added_pt);
      }
    }
...
void Preprocess::velodyne_handler(
  ...
  if (feature_enabled) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      pl_buff[layer].points.emplace_back(added_pt);
    }
  ...
  for (uint j = head; j < plsize; j++) {
    if (types[j].ftype == Poss_Plane || types[j].ftype == Real_Plane) {
      ...
      if (j == uint(last_surface + point_filter_num - 1)) {
        ...
        pl_surf.emplace_back(ap);

        last_surface = -1;
      }
    } else {
      if (types[j].ftype == Edge_Jump || types[j].ftype == Edge_Plane) {
        pl_corn.emplace_back(pl[j]);
      }
      if (last_surface != -1) {
        PointType ap;
...
int Preprocess::plane_judge(const PointCloudXYZI &pl, vector<orgtype> &types,
  ...
  for (i_nex = i_cur; i_nex < i_cur + group_size; i_nex++) {
    ...
    disarr.emplace_back(types[i_nex].dista);
  }
