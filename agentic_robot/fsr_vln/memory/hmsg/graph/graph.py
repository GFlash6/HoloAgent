...
class Graph:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        elif self.cfg.models.clip.type == "SigLIP-384":
        ...
        if self.cfg.main.use_gpt:
            ...
            self.client, self.gpt_model = _create_gpt_client()
            ...
            print("Dataset name: ", self.cfg.main.dataset)
            if self.cfg.main.dataset == "hm3d":
                self.dataset = HM3DSemDataset(dataset_cfg)
            elif self.cfg.main.dataset == "scannet":
                self.dataset = ScannetDataset(dataset_cfg)
            elif self.cfg.main.dataset == "horizon" or self.cfg.main.dataset == "g1":
                self.dataset = HorizonDataset(dataset_cfg)
            elif self.cfg.main.dataset == "pocket":
                # use replica dataset for pocket since they have similar data
                # format
                self.dataset = HorizonDataset(dataset_cfg)
            ...
            if self.cfg.main.dataset == "hm3d":
                ...
                self.dataset = ScannetDataset(dataset_cfg)
            elif self.cfg.main.dataset == "horizon" or self.cfg.main.dataset == "g1" or self.cfg.main.dataset == "pocket":
                self.dataset = HorizonDataset(dataset_cfg)
            elif self.cfg.main.dataset == "iphone":
                self.dataset = IPhoneDataset(dataset_cfg)
            elif self.cfg.main.dataset == "replica":
    ...
    def segment_floors_manually(self, path, flip_zy=False, mid_points=[]):
        ...
        for i in range(len(clustred_peaks) - 1):
            ...
            if clustred_peaks[i + 1] - clustred_peaks[i] >= 5.0:
                # 在两个峰值之间插入一个虚拟边界
    ...
    def segment_hmsg_room(self, floor: Floor, path):
        ...
        # import pdb; pdb.set_trace()
        if pcd_2d.shape[0] == 0:
            return  # 或 raise ValueError("pcd_2d is empty")
        ...
        # max(pcd_2d[:, 1]) min(pcd_2d[:, 1]) max(pcd_2d[:, 0]) min(pcd_2d[:,
        # 0])
        print("Max and min of pcd_2d: ", np.max(pcd_2d[:, 1]), np.min(
            pcd_2d[:, 1]), np.max(pcd_2d[:, 0]), np.min(pcd_2d[:, 0]))
        if self.cfg.pipeline.save_intermediate_results:
            # plot the histogram
            plt.figure()
            plt.imshow(
    ...
    def segment_objects(self, save_dir: str = None):
        ...
        for f_idx, floor in pbar:
            ...
            if len(floor.rooms) == 0:
                print(
                    "warning: floor {} has no rooms; skipping object assignment".format(
                        f_idx
                    )
                )
                continue
            ...
            for obj_floor_idx, mask_idx in obj_pbar:
                ...
                object = Object(
                    parent_room.room_id + "_" +
                    str(parent_room.object_counter),
                    parent_room.room_id,
                )
    ...
    def segment_hmsg_objects(self, save_dir: str = None):
        ...
        #         pcd, eps=0.05, min_points=10)
        for i, pcd in tqdm(
            enumerate(self.mask_pcds),
            total=len(self.mask_pcds),
            desc="Denoising mask_pcds",
        ):
            self.mask_pcds[i] = pcd_denoise_dbscan(
                pcd, eps=0.05, min_points=10)
        text_feats, classes = get_label_feats(
        ...
        for f_idx, floor in pbar:
            ...
            if len(floor.rooms) == 0:
                print(
                    f"warning: floor {f_idx} has no rooms; skipping object assignment")
                continue
            objects_inside_floor = list()
            # assign objects to rooms
            for i, pcd in enumerate(self.mask_pcds):
    ...
    def set_room_names(self, room_names: List[str]):
        ...
        """
        print("room_names: ", len(room_names))
        print("self.rooms: ", len(self.rooms))
    ...
    def query_hmsg_object(
        ...
        if query_method == "clip":
            ...
            for i in top_index:
                print(
                    "object id, object name, score: ",
                    objects_list[i].object_id,
                    objects_list[i].name,
                    sim_mat[query_id][i])
    ...
    def query_hierarchy_protected_icra(
        ...
        if use_gpt:
            ...
            return (
                ...
                [self.floors[floor_id].rooms[k] for k in room_ids] if floor_id != -
                1 else [self.rooms[k] for k in room_ids],
    ...
    def query_hierarchy_protected(
        ...
        if parsed_query is None:
            ...
            floor_query, room_query, object_query = parsed_query
            llm_parse_time = 0.0
        # log these in a txt file
        with open("room_obj_query_log.txt", "a") as f:
            f.write(
        ...
        res_dict = dict()
        res_dict["floor_query"] = floor_query
        res_dict["object_query"] = object_query
        res_dict["room_query"] = room_query
        res_dict["negative_labels"] = negative_labels
        res_dict["LLM_Parse_Time"] = llm_parse_time
