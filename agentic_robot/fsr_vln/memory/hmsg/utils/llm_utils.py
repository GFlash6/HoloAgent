...
def _create_gpt_client() -> Tuple[OpenAI, str]:
...
def infer_floor_id_from_query(floor_ids: List[int], query: str) -> int:
    ...
    client, model = _create_gpt_client()
    question = f"""
...
Please answer the floor number in one integer.
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a floor detector. You can infer the floor number based on a query.",
            },
            {
                "role": "user",
                "content": question,
