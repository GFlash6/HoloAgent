...
class VanillaMapper():
    """This class uses GT camera posses to generate a vanilla point-cloud
    reconstruction by unprojecting depths."""
    ...
    def __init__(self, config: dict, cam_intrinsics: torch.Tensor) -> None:
        ...
        self.voxel_size = config["mapping"].get("voxel_size", 0.01)  # 读取体素大小

        self.match_distance_th = 0.05  # 3 cm
        self.max_id = 0
        ...
        self.pcd_ids = torch.empty(
            (0, 1), device=self.device, dtype=torch.int32)
        self.pcd_obj_ids = torch.empty(
            (0, 1), device=self.device, dtype=torch.int32)
        self.pcd_colors = torch.empty(
            (0, 3), device=self.device, dtype=torch.uint8)
        ...
        if k_size > 1:
            pooling = torch.nn.MaxPool2d(
                ...
                padding=int(
                    k_size / 2))
            self.pooling = lambda mask: ~(
                pooling((~mask[None]).float(), )[0].bool())
        else:
            self.pooling = torch.nn.Identity()
        downscale_ratio = config["mapping"].get("downscale_res", 1)
    ...
    def map(self, frame_data: List[Any], c2w: torch.Tensor) -> None:
        ...
        if self.max_id > 0:
            ...
            frustum_mask = geometry_utils.compute_frustum_point_ids(
                self.pcd, camera_frustum_corners, device=self.device)
            _, matches = geometry_utils.match_3d_points_to_2d_pixels(depth, torch.linalg.inv(
                c2w), self.pcd[frustum_mask], self.cam_intrinsics, self.match_distance_th)
            # Do not project depth on points alredy matched
        ...
        depth, mask, image = self.downscale(
            depth), self.downscale(mask), self.downscale(image)

        x = x[mask]
        y = y[mask]
        depth = depth[mask]
        # convert to 3D
        x_3d = (x - self.cam_intrinsics[0, 2]) * \
            depth / self.cam_intrinsics[0, 0]
        y_3d = (y - self.cam_intrinsics[1, 2]) * \
            depth / self.cam_intrinsics[1, 1]
        z_3d = depth

        points = torch.hstack((x_3d.reshape(-1, 1), y_3d.reshape(-1, 1),
                              z_3d.reshape(-1, 1), torch.ones((x_3d.shape[0], 1), device=self.device)))
        points = torch.einsum("ij,mj->mi", c2w, points)

        first_p_idx = self.max_id
        last_p_idx = self.max_id + points.shape[0]

        self.pcd = torch.vstack((self.pcd, points[:, :3]))
        self.pcd_ids = torch.vstack(
            (self.pcd_ids,
             torch.arange(
                 first_p_idx,
                 last_p_idx,
        ...
        self.pcd_obj_ids = torch.vstack((self.pcd_obj_ids, torch.ones(
            (points.shape[0], 1), device=self.device, dtype=torch.int32) * -1))
        self.pcd_colors = torch.vstack((self.pcd_colors, torch.from_numpy(
            image.astype(np.uint8)).to(self.device)[mask].reshape(-1, 3)))
        self.max_id = last_p_idx
        self.kfs[frame_id] = {
