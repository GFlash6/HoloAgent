...
class WrapperORBSLAM2(VanillaMapper):
    """This class uses ORB-SLAM 2 to estimate camera posses and generates a
    vanilla point-cloud reconstruction by unprojecting depths."""

    def __init__(self,
                 config: Dict[str,
                              Any],
                 cam_intrinsics: torch.Tensor,
                 world_ref=torch.eye(4)) -> None:
        ...
        if (configs_path /
            config["dataset_name"].lower() /
                f"{config['data']['scene_name']}.yaml").exists():
            orbslam_config_path = configs_path / \
                config["dataset_name"].lower(
                ) / f"{config['data']['scene_name']}.yaml"
        else:
            orbslam_config_path = configs_path / \
                f"{config['dataset_name']}.yaml"

        self.orbslam2 = orbslam2.System(
            str(vocab_path),
    ...
    def track_camera(self, frame_data: List[Any]) -> None:
        ...
        tframe = frame_id
        # This actually blocks untill tracking is completed
    ...
    def map(self, frame_data, c2w) -> None:
        ...
        ):  # If tracking and maping are parallelized, this call would have a racing condition with ORB-SLAM2 tracking thread
        ...
        # LC and GBA happen one after the other, we could save some computation
        # detecting only GBA
        if self.close_loops and last_big_change_id != self.last_big_change_id:
            self.last_big_change_id = last_big_change_id
            # DEBUG:
            print(
                f" Least seen object before: {torch.unique(self.pcd_obj_ids, return_counts=True)[1].min()}")
            self.update_map()
            # DEBUG:
            print(
                f" Least seen object after: {torch.unique(self.pcd_obj_ids, return_counts=True)[1].min()}")
    ...
    def update_map(self):
        ...
        for updated_kf in updated_kfs:
            ...
            if kf is None:
                ...
                # If a Keyframe is added/tracked after orb_slam starts LC/GBA,
                # is it going to be in the retrieved list of kfs?
                continue

            kf_c2w = self.estimated_c2ws[kf["id"]]
            updated_kf_c2w = self.world_ref @ convert_pose(
                updated_kf[1:13], device=self.device)
            ...
            # Measure mIoU and number of fused instances to evaluate reduced
            # approach.
            updated_kf_pcd = torch.einsum('mn,bn->bm',
                                          transform,
                                          torch.cat([self.pcd[kf["pcd_idxs"][0]:kf["pcd_idxs"][1]],
                                                     torch.ones((kf["pcd_idxs"][1] - kf["pcd_idxs"][0],
                                                                 1),
                                                                device=self.device)],
                                                    dim=1))[:,
                                                            :3]

            # kfs that are not in updated_kfs were pruned by ORB_SLAM. They
            # will be removed together with their associated pcd
            ...
            new_pcd_ids.append(
                self.pcd_ids[kf["pcd_idxs"][0]:kf["pcd_idxs"][1]])
            new_pcd_obj_ids.append(
                self.pcd_obj_ids[kf["pcd_idxs"][0]:kf["pcd_idxs"][1]])
            new_pcd_colors.append(
                self.pcd_colors[kf["pcd_idxs"][0]:kf["pcd_idxs"][1]])
