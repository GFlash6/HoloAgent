...
class CLIPGenerator:
    def __init__(self, config: Dict, device: str = "cuda"):
        ...
        if self.embed_type == "learned":
            ...
            pt_path = os.path.join(
                config["weights_predictor_path"], "model.pt")

            model_weights = torch.load(
                pt_path, map_location=self.device, weights_only=False)
            self.clips_fusion_model = WeightsPredictorMerger(
                model_config["model"]).eval()
            self.clips_fusion_model.load_state_dict(model_weights)

            self.clips_fusion = lambda clip_g, clip_seg, clip_bbox: self.clips_fusion_model(
                torch.cat([clip_g[:, None], clip_seg[:, None], clip_bbox[:, None]], dim=1))
            ...
            self.clips_fusion = lambda clip_g, clip_seg, clip_bbox: clip_utils.fuse_clips(
                clip_g, clip_seg, clip_bbox, self.embed_type, w_masked, w_global)
    ...
    def cuda(self, device: str = "cuda") -> None:
        """Move predictor model to a specified cuda device."""
        self.device = device
        self.model.to(device)
        self.similarity_args = [x.to(device) for x in self.similarity_args]
        if self.embed_type == "learned":
            self.clips_fusion_model.to(device)
    ...
    def extract_clip(
        ...
        if self.embed_type != "vanilla":
            if len(image.shape) == 3:
                image = image[None, ...]
            clip_g = torch.nn.functional.normalize(
                self.encode_image(image), p=2, dim=-1)

        if len(seg_images) == 0:
            return torch.tensor([], device=self.device)

        if self.embed_type == "vanilla":
            clip_embed = torch.nn.functional.normalize(
                self.encode_image(seg_images[:, :3]), p=2, dim=-1)
        else:
            n_clips = seg_images.shape[0]
            seg_images = torch.vstack([seg_images[:, :3], seg_images[:, 3:]])
            clip_seg = torch.nn.functional.normalize(
                self.encode_image(seg_images), p=2, dim=-1)

            if return_all:
                clip_embed = torch.cat([clip_g.repeat(n_clips, 1)[
                                       :, None], clip_seg[:n_clips][:, None], clip_seg[n_clips:][:, None]], dim=1)
            else:
                clip_embed = self.clips_fusion(clip_g.repeat(
                    n_clips, 1), clip_seg[:n_clips], clip_seg[n_clips:,])
    ...
    def get_txt_embedding(self, text_list: List[str]) -> torch.Tensor:
        ...
        Each element of the list is tokenized individually Args:
        ...
        tok_phrases = torch.cat([self.tokenizer(phrase)
                                for phrase in text_list]).to(self.device)
        embeds = self.model.encode_text(tok_phrases)
        embeds /= embeds.norm(dim=-1, keepdim=True)
        return embeds

    @torch.no_grad
    def get_embed_txt_similarity(
            self,
            ins_descriptors: torch.Tensor,
            txt_queries: List[str],
            templates: str | List[str] = ['{}']) -> torch.Tensor:
        """
        Computes independently the similarity between image embeddings and text
        queries.
        ...
        txt_embeds = torch.zeros(
            (n_queries,
             ins_descriptors.shape[1]),
            device=ins_descriptors.device)
        if isinstance(templates, str):
            templates = [templates]
        queries = [[template.format(query) for template in templates]
                   for query in txt_queries]

        for j in range(n_queries):
            # Compute the text embedding of each query individually to make
            # them independent from other queries
            embed = self.get_txt_embedding(
                queries[j]).mean(
                0, keepdim=True).float()
            txt_embeds[j] = torch.nn.functional.normalize(embed, p=2, dim=-1)

        sim_map = self.get_similarity(
            txt_embeds, ins_descriptors, *self.similarity_args)
        return sim_map
