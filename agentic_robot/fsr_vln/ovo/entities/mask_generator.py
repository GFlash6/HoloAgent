...
class MaskGenerator:
    ...
    def __init__(self,
        ...
        if scene_name:
            self.masks_path = os.path.join(
                config["masks_base_path"], scene_name)
        else:
            assert not config.get(
                "precompute", False), "To precompute masks or use precomputed masks \"scene_name\" is required!"
            self.masks_path = ""
        ...
        self.stability_score_thresh = config.get(
            "stability_score_thresh", 0.90)
        self.min_mask_region_area = config.get("min_mask_region_area", 100)
        self.device = device
        if (self.precomputed or config.get("precompute", False)
            ) and os.path.isdir(self.masks_path):
            print(
                " {} path already exists, skipping masks precompute! To recompute masks delete old masks!".format(
                    self.masks_path))
    ...
    def visualize_masks(
            ...
            binary_maps: torch.Tensor) -> np.ndarray:
        """
        Visualize segmentation masks on the original image.

        Args:
            - image (np.ndarray): The original input image (H, W, 3).
            - seg_map (torch.Tensor): Segmentation map of shape (H, W), where each pixel value indicates the mask ID.
            - binary_maps (torch.Tensor): Binary maps of shape (N, H, W), where N is the number of masks.

        Returns:
            - annotated_image (np.ndarray): The image with segmentation masks visualized.
        """
        # Convert image to BGR format for OpenCV
        annotated_image = image.copy()

        # Generate random colors for each mask
        num_masks = binary_maps.shape[0]
        colors = [tuple(np.random.randint(0, 255, 3).tolist())
                  for _ in range(num_masks)]

        # Overlay each mask on the image
        for mask_id in range(num_masks):
            mask = binary_maps[mask_id].cpu().numpy().astype(
                np.uint8)  # Convert to NumPy array
            contours, _ = cv2.findContours(
                mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            for contour in contours:
                cv2.drawContours(
                    annotated_image, [contour], -1, colors[mask_id], thickness=2)

            # Fill the mask area with transparency
            mask_indices = mask > 0
            annotated_image[mask_indices] = (
                0.5 *
                annotated_image[mask_indices] +
                0.5 *
                np.array(
                    colors[mask_id])).astype(
                np.uint8)
    ...
    def get_masks(self, image: np.ndarray, frame_id: int = None):
        ...
        return torch.from_numpy(seg_map).to(
            self.device), torch.from_numpy(binary_maps).to(
            self.device)
    ...
    def segment(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        with torch.autocast(
            ...
            seg_map, binary_maps = segment_utils.mask2segmap(
                masks_default, image)
    ...
    def precompute(
            ...
            segment_every: int) -> None:
        """
        Precomputes segmentation masks for the given dataset and frame IDs.
        This method checks if the segmentation masks for the specified frame
        IDs already exist. If they do not, it generates the masks using the
        mask generator and saves them to the specified path.
        ...
        for frame_id in frame_ids:
            ...
            map_path = os.path.join(
                self.masks_path,
                f"{frame_id:04d}_seg_map_default.npy")
            bmap_path = os.path.join(
                self.masks_path,
                f"{frame_id:04d}_bmap_default.npy")
    ...
    def _save_masks(
            ...
            frame_id: int) -> None:
        """
        Saves the segmentation map and binary maps of a fiven frame to
        self.masks_path.
        ...
        map_path = os.path.join(
            self.masks_path,
            f"{frame_id:04d}_seg_map_default")
        np.save(map_path, seg_map)

        bmap_path = os.path.join(
            self.masks_path,
            f"{frame_id:04d}_bmap_default")
    ...
    def _load_masks(self, frame_id: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if os.path.exists(map_path):
            ...
            # TODO: store and load original binary maps rather than compute
            # them from the saved filtered seg map
            binary_path = os.path.join(
                self.masks_path, f"{frame_id:04d}_bmap_default.npy")
            if os.path.exists(binary_path):
                binary_maps = np.load(binary_path)
            else:
                idxs = np.arange(seg_map.max() + 1)
                binary_maps = np.repeat(np.expand_dims(seg_map, 0), len(
                    idxs), axis=0) == np.expand_dims(idxs, [-1, -2])
        else:
    ...
    def get_seg_img(self, binary_maps: torch.Tensor,
                    image: torch.Tensor) -> torch.Tensor:
        """
        For each binary map, compute the corresponding segmented image without
        background, reshaped at size self.mask_res. If self.multi_crop, a
        version with background is also computed and concatenated on the color
        dimension.
        ...
        """
        return segment_utils.segmap2segimg(
            binary_maps, image, self.multi_crop, out_l=self.mask_res)

    def get_precomputed_seg_img(
            self,
            binary_maps: torch.Tensor,
            best_map_idxs: torch.Tensor,
            changed_binary_maps: torch.Tensor,
            image: torch.Tensor,
            frame_id: int = None) -> torch.Tensor:
        """
        Loads precomputed segmentation images for a given frame ID.

        Given that seg_images are precomputed outside of OVO, these may need to be update after binary_maps fusion. Args:
            - binary_maps (tensor): A tensor of (N, H, W) containing N binary maps.
        ...
        if len(changed_binary_maps) > 0:
            seg_images[changed_binary_maps] = segment_utils.segmap2segimg(
                binary_maps[changed_binary_maps], image, self.multi_crop, out_l=self.mask_res)
    ...
    def _load_seg_imgs(self, frame_id: int) -> torch.Tensor:
        ...
        img_path = os.path.join(
            self.masks_path,
            f"{frame_id:04d}_seg_imgs_default.npy")
