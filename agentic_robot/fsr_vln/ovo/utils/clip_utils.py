...
def siglip_cosine_similarity(
        ...
        logit_bias: float) -> torch.Tensor:
    p = txt_embeds.to(img_embed.dtype)
    logits = torch.mm(img_embed, p.T) * logit_scale.exp() +\
        logit_bias  # obj x phrases
...
def clip_cosine_similarity(
        txt_embeds: torch.Tensor,
        img_embed: torch.Tensor) -> torch.Tensor:
    p = txt_embeds.to(img_embed.dtype)
...
def fuse_clips(
        ...
        embed_type: str,
        w_masked: float,
        w_global: float) -> torch.Tensor:
    cos = torch.nn.CosineSimilarity(dim=-1, eps=1e-6)
    if embed_type == "hovsg" or embed_type == "fixed_weights":
        w_local = w_masked
        clip_l = torch.nn.functional.normalize(
            clip_seg * w_local + clip_bbox * (1 - w_local), p=2, dim=-1)
        ...
        clip_embed = torch.nn.functional.normalize(
            clip_g * w_global + clip_l * (1 - w_global), p=2, dim=-1)
        ...
        # start with 0.18
        w_global = (cos(clip_g, clip_l) * w_global).unsqueeze(-1)
        clip_embed = torch.nn.functional.normalize(
            clip_g * w_global + clip_l * (1 - w_global), p=2, dim=-1)
    elif embed_type == "concept_fusion":

        similarity_scores = cos(clip_g, clip_bbox)
        w_global = torch.nn.functional.softmax(
            similarity_scores, dim=0).unsqueeze(-1)
        clip_embed = torch.nn.functional.normalize(
            w_global * clip_g + (1 - w_global) * clip_bbox, p=2, dim=-1)
    else:
        # vanilla
        clip_embed = clip_seg
    return clip_embed
...
def load_clip_model(
    ...
    tf_to_keep = [
        tf for tf in preprocess.transforms if isinstance(
            tf,
            Resize) or isinstance(
            tf,
            CenterCrop) or isinstance(
                tf,
            Normalize)]
